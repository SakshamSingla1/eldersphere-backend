...
public class AuthService {
    ...
    /** Roles allowed for self-registration on the user-facing API */
    ...
    public AuthResponse register(RegisterRequest request) throws ElderSphereException {
        log.info("User registration: {}", request.getEmail() != null ? request.getEmail() : request.getPhone());
        ...
        // Self-referencing audit on first save
    ...
    public AuthResponse refreshToken(String refreshToken) throws ElderSphereException {
        ...
        User user = userDao.findByIdOrThrow(Long.parseLong(userIdStr));
    ...
    private User resolveUser(String identifier) {
        if (identifier.contains("@")) return userDao.findByEmail(identifier);
    ...
    private void handleFailedAttempt(UserSecuritySettings settings) throws ElderSphereException {
        ...
        if (attempts >= MAX_FAILED_ATTEMPTS) {
            settings.setLockedUntil(java.time.Instant.now().plusSeconds(15 * 60));
            log.warn("Account locked after {} failed attempts for user id={}", attempts, settings.getUser().getId());
        }
        userSecuritySettingsDao.saveSettings(settings);
    }
}
