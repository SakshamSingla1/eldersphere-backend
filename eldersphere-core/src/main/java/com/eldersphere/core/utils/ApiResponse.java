...
public class ApiResponse {
    ...
    public static <T> ResponseEntity<ResponseModel<T>> failureResponse(T data, ElderSphereException elderSphereException ) {
        return ResponseEntity.badRequest().body(
                new ResponseModel<T>(
                        elderSphereException.getMessage(),
                        data, elderSphereException.getExceptionCode()).toFailure());
    }
    ...
    public static <T> ResponseEntity<ResponseModel<T>> respond(T data, String successMessage, ElderSphereException  elderSphereException) {
        if (data != null && !data.equals(false)) {
            return ApiResponse.successResponse(data, successMessage);
        } else {
            return ApiResponse.failureResponse(data, elderSphereException);
        }
    }
    ...
    public static <T> ResponseEntity<ResponseModel<T>> respond(T data, String successMessage, ElderSphereException elderSphereException, boolean isFailure) {
        if(!isFailure) {
            return ApiResponse.successResponse(data, successMessage);
        } else {
            return ApiResponse.failureResponse(data, elderSphereException);
